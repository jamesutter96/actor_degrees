...
def shortest_path(source, target):
    ...
    """
    # intitilizing all the variables 
    path     = []
    frontier = QueueFrontier()
    explored = set()

    # intitilizing the starting node for exploration 
    ...
    while True:
        # if the frontier is empty then there is no solutuion 
        if frontier.empty() == True:
            raise Exception('No Solution')
        # pulling the node from the frontier 
        node = frontier.remove()

        if node.state == target:
            solution = []
            # back track using the parent nodes 
            while node.parent is not None:
                solution.append((node.action, node.state))
                node = node.parent

            # reverse the back tracking to get the correct order
            solution.reverse()
        ...
        # adding the neightbors of the node to the frontier 
